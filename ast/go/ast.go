...
func main() {
	...
	if err := os.MkdirAll(outputDir, 0755); err != nil {
		panic(fmt.Sprintf("Could not create output directory: %v", err))
	}

	// Read test files
	fileA, err := readFile("../example/a.tst")
	if err != nil {
		panic(fmt.Sprintf("Could not read example/a.tst: %v", err))
	}

	fileB, err := readFile("../example/b.tst")
	if err != nil {
		panic(fmt.Sprintf("Could not read example/b.tst: %v", err))
	}

	fileC, err := readFile("../example/c.tst")
	if err != nil {
		panic(fmt.Sprintf("Could not read example/c.tst: %v", err))
	}

	var parseTotal float64
	var marshalTotal float64
	iteration := 0

	parseFile := func(fileContents, outputFilename string) {
		start := time.Now()
		...
		if err != nil {
			panic(fmt.Sprintf("Could not serialize AST: %v", err))
		}
		end := time.Now()

		// Write to file
		outputPath := filepath.Join(outputDir, outputFilename)
		if err := os.WriteFile(outputPath, astJSON, 0644); err != nil {
			panic(fmt.Sprintf("Could not write AST file: %v", err))
		}

		durationParseMs := float64(endParse.Sub(start).Nanoseconds()) / 1e6
		durationMarshalMs := float64(end.Sub(endParse).Nanoseconds()) / 1e6
		parseTotal += durationParseMs
		marshalTotal += durationMarshalMs
	...
	if err != nil {
		panic(fmt.Sprintf("Could not marshal results: %v", err))
	}
